...
import { getRequest, getSuccess, getPaginationState, getEducationDetailedSuccess, getEducationSuccess, getError } from './educationSlice';
...
export const educationDelete = (id) => async (dispatch) => {
  ...
  dispatch(getRequest());
  try {
    const response = await axiosInstance.delete(`/admin/education-materials/${id}/`);
...
export const getEducationById = (id) => async (dispatch) => {
  const axiosInstance = useAxios();
  try {
    const result = await axiosInstance.get(`/admin/education-materials/${id}/`);
    console.log(result.data);
    if (result.data) {
      dispatch(getEducationDetailedSuccess(result.data));
    }
  } catch (error) {
    dispatch(getError(error.data));
    console.log(error);
